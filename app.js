...
form.addEventListener('submit', (e) => {
  e.preventDefault();        // by default form submit value to the server so we need to prevent it!
  ...
  if (isNaN(val) || val <= 0 || val > 9)
  {
    ...
    let newTextArr = text.slice(0, val);  // to get a copy of paticular section of the original array
    
    newTextArr = newTextArr.map((item) => {
      return `<p class="result">${item}</p>`;
    }).join('');

    result.innerHTML = newTextArr;
  }
});